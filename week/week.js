...
let GenerateTask = () => {
    ...
    newElement.innerHTML = Week.map((x) =>{
    ...
    countings.innerText = Week.map((x) => x.item).reduce((x, y) => x+y,0)
    ...
    for (let i = 0; i < tas_K.length; i++) {
        ...
        donetask.onclick = (event) => {
            ...
            let todayCounts = document.getElementById('week');
            todayCounts.innerText = Week.map((x) => x.item).reduce((x, y) => x+y,0)
