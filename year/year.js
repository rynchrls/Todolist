...
let GenerateTask = () => {
    ...
    newElement.innerHTML = Year.map((x) =>{
    ...
    countings.innerText = Year.map((x) => x.item).reduce((x, y) => x+y,0)
    ...
    for (let i = 0; i < tas_K.length; i++) {
        ...
        donetask.onclick = (event) => {
            ...
            let todayCounts = document.getElementById('year');
            todayCounts.innerText = year.map((x) => x.item).reduce((x, y) => x+y,0)
