...
let GenerateTask = () => {
    ...
    newElement.innerHTML = Month.map((x) =>{
    ...
    countings.innerText = Month.map((x) => x.item).reduce((x, y) => x+y,0)
    ...
    let weekCounts = document.getElementById('week');
    weekCounts.innerText = Week.map((x) => x.item).reduce((x, y) => x+y,0)
    ...
    for (let i = 0; i < tas_K.length; i++) {
        ...
        donetask.onclick = (event) => {
            ...
            let todayCounts = document.getElementById('month');
            todayCounts.innerText = Month.map((x) => x.item).reduce((x, y) => x+y,0)
